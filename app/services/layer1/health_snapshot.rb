...
module Layer1
  class HealthSnapshot
    ...
    TX_OUTPUTS_ASYNC_QUEUE = "tx_outputs_async"
    ...
    def call
      ...
      tx_outputs_async = tx_outputs_async_snapshot(processed)
...

      {
        ...
        buffers: buffers,
        tx_outputs_async: tx_outputs_async,
    ...
    def tx_outputs_async_snapshot(processed_height)
      worker = queue_process(TX_OUTPUTS_ASYNC_QUEUE)
      queue_size = queue_size(TX_OUTPUTS_ASYNC_QUEUE)
      scheduled_jobs = scheduled_queue_size(TX_OUTPUTS_ASYNC_QUEUE)
      table_exists = ActiveRecord::Base.connection.data_source_exists?("layer1_tx_output_syncs")
      ...
      return { enabled: false, worker: worker } unless configured
      ...
      pending_scope = Layer1TxOutputSync.where(status: %w[pending processing failed])
      last_synced_height = Layer1TxOutputSync.where(status: "synced").maximum(:height)

      {
        ...
        projection_lag_blocks:
          last_synced_height ? [processed_height.to_i - last_synced_height.to_i, 0].max : nil
      }
    rescue StandardError => e
      {
        enabled: true,
