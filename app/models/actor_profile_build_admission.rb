...
class ActorProfileBuildAdmission < ApplicationRecord
  STATUSES = %w[pending processing completed failed].freeze
  REASONS = %w[cluster_composition address_spend missing_profile recovery].freeze
  TRANSITIONS = {
  ...
  IDENTITY = %w[cluster_id cluster_composition_version source_height source_hash].freeze
  ...
  validates :source_hash, presence: true
  validates :reason, inclusion: { in: REASONS }
  validates :status, inclusion: { in: STATUSES }
  validates :attempts,
    numericality: { only_integer: true, greater_than_or_equal_to: 0 }
  validates :cluster_id,
    uniqueness: { scope: %i[cluster_composition_version source_height source_hash] }
  ...
  def identity_is_immutable
    errors.add(:base, "build identity is immutable") if
